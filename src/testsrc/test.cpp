...
int main (int argc, char** argv) {
  ...
  while ((sz = read(inkb, &ev, sizeof(struct input_event)))) {
    ...
    if (!((ev.value == 1) || (ev.value == 0)))
      continue;
    std::cout << "ev " << ev.code << " "<<ev.value<<" " << ev.type<< "\n";

    //    switch (ev.code) {
    // case KEY_A: {
    //   send_event(Uinput::getfd().fd, EV_KEY, KEY_A, 1);
    //   send_event(Uinput::getfd().fd, EV_KEY, KEY_A, 0);
    //   send_event(Uinput::getfd().fd, EV_SYN, SYN_REPORT, 0);      
    //   break;
    // }
    // default:
    //   send_event(Uinput::getfd().fd, EV_KEY, ev.code, ev.value);
    //   //      send_event(Uinput::getfd().fd, EV_KEY, ev.code, 0);
    //   send_event(Uinput::getfd().fd, EV_SYN, SYN_REPORT, 0);
